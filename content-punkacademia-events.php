...
<?php
	...
	$start_date = get_post_meta( $post->ID, 'punkacademia_events_start_date', true ); // Event start date
	$end_date = get_post_meta( $post->ID, 'punkacademia_events_end_date', true ); // Event end date
	$details = get_post_meta( $post->ID, 'punkacademia_events_details', true ); // Details for this event
...
?>
	...
	<article class="container">
				...
				<?php if ( $has_registration || $show_google_calendar || $show_ical_invite ) : ?>
					<p>
						...
						<?php if ( $show_ical_invite ) : ?>
							<a target="blank" rel="nofollow"  href="<?php the_permalink(); ?>?ical=<?php echo esc_attr( $post->ID ); ?>">+ <?php _e( 'iCal Invite', 'punkacademia' ); ?></a>
						<?php endif; ?>
					</p>
				<?php endif; ?>
			<?php else : ?>
				<p><em><?php _e( 'This event has ended.', 'punkacademia' ); ?></em></p>
			<?php endif; ?>
		</aside>
		...
		<?php edit_post_link( __( 'Edit', 'punkacademia' ), '<p>', '</p>' ); ?>
	...
	<article>
		...
		<a class="link-block" href="<?php the_permalink(); ?>">
			...
			<aside class="margin-bottom-small">
				<?php if ( $has_start_date ) : ?>
					<?php echo esc_html( date( 'D, M j, Y', $start_date ) ); ?> <?php if ( $has_start_time ) : ?><?php _e( 'at', 'punkacademia' ); ?> <?php echo esc_html( $details['time_start_hour'] . ':' . $details['time_start_minutes'] . ' ' . $details['time_start_ampm'] ); ?><?php endif; ?>
				<?php endif; ?>
				<?php if ( $has_start_date && $has_end_date && $end_date !== $start_date ) : ?>
					&ndash; <?php echo esc_html( date( 'D, M j, Y', $end_date ) ); ?> <?php if ( $has_end_time ) : ?><?php _e( 'at', 'punkacademia' ); ?> <?php echo esc_html( $details['time_end_hour'] . ':' . $details['time_end_minutes'] . ' ' . $details['time_end_ampm'] ); ?><?php endif; ?>
				<?php endif; ?>
		...
		<?php
			...
			if ( !Extras\punkacademia_is_last_post($wp_query) ) :
		?>
		    <hr>
		<?php endif; ?>
